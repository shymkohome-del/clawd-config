...
- Wrapper runs a local preflight parser step using `scripts/preflight-parse.sh` to validate branch naming and story `Status:` without network calls and emits a summary.
...
- `automerge.setting_disabled`: repo setting "Allow auto-merge" disabled
- `label-missing`: `automerge-ok` label missing on `story/*`
- `node-id-missing`: GraphQL node id not found for PR
- `pr-missing`: PR not found for given branch

## **Troubleshooting**

- See job summary at the end of runs for decisions and reasons
- Check annotations in logs for misconfigurations
 - GraphQL enable can fail if repo-level auto-merge is disabled; fallback merge-on-green will still merge when checks pass and labels allow.
...
- Preflight parser validates branch and story file presence without network calls
 - You can also run: `bash scripts/preflight-parse.sh $(git rev-parse --abbrev-ref HEAD)`