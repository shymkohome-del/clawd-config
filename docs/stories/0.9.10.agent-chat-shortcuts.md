...
- [x] (Optional) Add a helper script `scripts/story-from-file.sh` to output `story/<id>-<slug>` given `docs/stories/<id>.<slug>.md`.
- [x] (Optional) Reference the shortcuts in `docs/dev-qa-status-flow.md` for discoverability.
...
| ---- | ------- | ----------- | ------ |
| 2025-09-01 | 0.3 | QA: validation blocked (missing Dart/Flutter) | QA Agent |
| 2025-09-01 | 0.2 | Added helper script and cross-link to Dev ↔ QA flow | Dev Agent |
...
- Implemented:
  ...
  - Added `scripts/story-from-file.sh` helper script for branch derivation.
  - Cross-linked chat shortcuts in `docs/dev-qa-status-flow.md`.
- Pending: None
- File List:
  - `AGENTS.md` (updated)
  - `scripts/story-from-file.sh` (new)
  - `docs/dev-qa-status-flow.md` (updated)
