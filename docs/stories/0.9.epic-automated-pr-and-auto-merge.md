...
- Prefer REST/GraphQL calls over `gh` to avoid runner dependencies.
 - Legacy `.github/workflows/ci-auto-pr.yml` still exists and uses `gh`; consider disabling or aligning with REST-only path.
...
- Phase 1 (0.9.2): Partial. REST-only path implemented; legacy GH CLI remains in `ci-auto-pr.yml`.
...
- Phase 4 (0.9.5): Not Met. No reusable workflow or self-tests.
- Phase 5 (0.9.6): Partial. Policy files missing; docs updates pending.
