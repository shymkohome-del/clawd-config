...
| 2025-08-20 | 0.6 | QA: Reviewed — AC1, AC2, AC3 Pass; AC4 Partial. Request: invoke `scripts/preflight-parse.sh` in wrapper prior to reusable call. Status InProgress. | QA |
...
- AC1 — Reusable workflow: Pass. `.github/workflows/reusable-auto-pr.yml` exposes inputs/outputs via `workflow_call` and emits outputs (e.g., `pr_number`).
- AC2 — Wrapper: Pass. `.github/workflows/auto-pr-from-qa.yml` is a thin wrapper that calls the reusable via `uses:` with repo defaults; no duplicated PR logic.
- AC3 — Self-tests: Pass. `auto-pr-self-test.yml` runs a matrix (feature, story done, story not done) in dry‑run mode and prepares synthetic story files.
- AC4 — Local preflight: Partial. `scripts/preflight-parse.sh` exists, but the wrapper does not invoke it; add a preflight job to run the script against `${{ github.ref_name }}` before calling the reusable.

Verdict: Set `Status: InProgress`. Only AC4 remains; wire the preflight parser into the wrapper CI.
