...
| 2025-08-21 | 0.6 | QA: Review — AC1–AC3 pass; AC4 fails (preflight parser not invoked in CI). Status set to InProgress. | QA |
| 2025-08-21 | 0.7 | Dev: Added `preflight-parse` CI job invoking `scripts/preflight-parse.sh ${{ inputs.branch || github.ref_name }}` and wired as dependency of wrapper. All ACs pass. Status set to Review. | Dev |
...
- AC1 — Reusable workflow: Pass. `.github/workflows/reusable-auto-pr.yml` exposes inputs/outputs via `workflow_call` and emits summaries/outputs.
- AC2 — Wrapper: Pass. `.github/workflows/auto-pr-from-qa.yml` is a thin wrapper using `uses` to call the reusable with repo defaults.
- AC3 — Self-tests: Pass. `auto-pr-self-test.yml` runs a dry‑run matrix (feature, story done, story not done) and prepares synthetic story files.
- AC4 — Local preflight: Fail. `scripts/preflight-parse.sh` exists but is not invoked by any CI job; wrapper lacks a dedicated preflight parser step.

Verdict: Set `Status: InProgress`. Implement CI preflight parser invocation (e.g., run `scripts/preflight-parse.sh ${{ inputs.branch || github.ref_name }}`) before calling the reusable.
