...
- [ ] Remove any remaining `gh` usage; keep `curl`+`jq` REST/GraphQL calls.
- [ ] Ensure the PR creation step first queries `/pulls?head=owner:branch&state=open` and reuses if found.
- [ ] Echo `pr_number=<number>` to `$GITHUB_OUTPUT` unconditionally (create or reuse path).
- [ ] Validate branch and story ID parsing; fail with clear taxonomy when unsupported or missing.
- [ ] Add step to apply label `automerge-candidate` using REST API.
- [ ] Keep gating: only execute on `story/*` when the story file includes `Status: Done`; otherwise skip with reason.
...
- AC1 — REST/GraphQL only: Partial. `auto-pr-from-qa.yml` uses `curl` + `jq` (no `gh`). Legacy workflow `ci-auto-pr.yml` still uses `gh` for PR ops.
- AC2 — Idempotency: Pass. Prior to creation, existing PR is queried via `/pulls?head=owner:branch&state=open` and reused.
- AC3 — Output `pr_number`: Pass. Step writes `pr_number` to `$GITHUB_OUTPUT` and is consumed later.
- AC4 — Titles/bodies consistent: Pass. `story/*` branches parse story ID; non-story use generic title.
...
Verdict: Keep `Status: InProgress`. Next action: Remove or refactor `ci-auto-pr.yml` to use REST/GraphQL only, or disable it in favor of `auto-pr-from-qa.yml`.
