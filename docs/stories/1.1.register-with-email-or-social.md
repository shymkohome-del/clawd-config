...
Re-check (2025-09-01):
- Evidence:
  - Frontend flows for email/social registration implemented; success navigates to Home
  - Principal mapping currently via frontend shim (`FEATURE_PRINCIPAL_SHIM`) in `ICPService` → not persisted server-side
  - No `canisters/` or backend principal mapping code present in repo

- Acceptance Criteria:
  - [x] AC1: Registration creates session and navigates → Met via UI and service stubs
  - [ ] AC2: Principal mapping associated with account → NOT met (shim only)

- Actions to pass next iteration:
  - Implement backend principal mapping in User Management canister (see Story 0.11)
  - Replace shim path with real canister calls; update tests to assert principal on returned User
  - Document feature flag removal plan once backend is available

- Status Decision: InProgress until real principal mapping is integrated
