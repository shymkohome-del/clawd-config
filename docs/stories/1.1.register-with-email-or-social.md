...
- [x] Frontend: Add registration UI in `lib/features/auth/` with email/password and social login entry points (Google, etc.). (AC: 1)
  ...
  - [x] **Logging: Implement debug logging for user registration attempts, OAuth flow steps, form validation events, authentication state changes, and principal mapping operations. Ensure logging is disabled in production flavors and only active in development/staging builds.**
...
- [x] Backend: Ensure User Management canister exposes `register`, `loginWithOAuth`, and principal mapping. (AC: 2)
...
- [x] Security: Validate inputs and enforce rate limits where applicable. (AC: 1)
...
| 2025-09-01 | 1.1 | Marked backend and security tasks complete in story doc; no functional changes | Dev |
...
Acceptance review (2025-08-27):
- AC1 — Pass: Static analysis clean (0 issues); all 19 unit/widget/integration tests pass. Email and social flows sign in and navigate to Home; localization verified for EN/LV labels/errors. RegisterScreen properly validates inputs and handles OAuth flows.
- AC2 — Pass: Backend principal mapping fully implemented in Motoko canister `canisters/user_management/src/main.mo`. Both `register()` and `loginWithOAuth()` functions use caller principal as `User.id` and persist mapping in HashMap. ic-repl test validates register, duplicate prevention, OAuth login, and profile fetch by principal. Candid interface specifies `id: principal` type correctly.

## Dev Agent Record

### Test Notes
- Testing workflow automation after jq fixes
- Validating end-to-end QA → auto-merge flow
