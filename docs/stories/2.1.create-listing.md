...
- [ ] Frontend: Add Create Listing UI in `lib/features/market/` with validation and submission (AC: 1)
  - [ ] Create `CreateListingScreen` with form fields and validators. [Source: architecture/frontend-architecture.md#flutter-app-structure]
  - [ ] Implement Bloc/Cubit for listing creation state, loading, and error handling. [Source: architecture/frontend-architecture.md#state-management-architecture]
  - [ ] Integrate image picker and IPFS upload flow (service abstraction). [Source: architecture/core-workflows.md#create-listing-flow]
  - [ ] **Localization: Implement multi-language support for form labels, field placeholders, validation messages, category names, condition types, and submission confirmations.**
  - [ ] **Logging: Implement debug logging for listing creation attempts, form validation errors, image upload progress, IPFS operations, and listing submission events. Ensure logging is disabled in production flavors and only active in development/staging builds.**

- [ ] Frontend: Service layer integration to Marketplace canister (AC: 1)
  - [ ] Add `MarketplaceService.createListing(...)` call through ICP service actor. [Source: architecture/frontend-architecture.md#frontend-services-layer]
  - [ ] Map canister `Result` to domain response/errors for UI. [Source: architecture/coding-standards.md#critical-fullstack-rules]

- [ ] Backend: Ensure Marketplace canister supports listing creation with validation and indexes (AC: 1)
  - [ ] Confirm/define Candid for `createListing`, `getListings`, `getListing`. [Source: architecture/api-specification.md#icp-candid-interface]
  - [ ] Input validation and rate limiting in canister. [Source: architecture/backend-architecture.md#canister-based-service-architecture] [Source: architecture/coding-standards.md#critical-fullstack-rules]
  - [ ] Maintain indexes for user listings and category search. [Source: architecture/backend-architecture.md#canister-based-service-architecture]

- [ ] Security: Authentication and principal checks (AC: 1)
  - [ ] Require authenticated principal to create listing; ensure caller recorded as seller. [Source: architecture/backend-architecture.md#authentication-architecture]
  - [ ] Enforce rate limits for user-facing functions. [Source: architecture/coding-standards.md#critical-fullstack-rules]
