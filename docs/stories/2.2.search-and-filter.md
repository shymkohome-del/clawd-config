...
- [ ] Frontend: Search and filter UI (AC: 1)
  - [ ] Create `SearchScreen` with search input and filter widgets. [Source: architecture/frontend-architecture.md#flutter-app-structure]
  - [ ] Add filter options (price range, category, condition, location, etc.). [Source: architecture/data-models.md#listing-model]
  - [ ] Implement search results display with pagination. [Source: architecture/frontend-architecture.md#flutter-app-structure]
  - [ ] **Localization: Implement multi-language support for search placeholders, filter labels, category names, condition types, sorting options, and "no results" messages.**
  - [ ] **Logging: Implement debug logging for search queries, filter selections, pagination events, search result counts, and performance metrics for search operations. Ensure logging is disabled in production flavors and only active in development/staging builds.**
