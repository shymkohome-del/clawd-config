...
2. Given audit logs exist, when I query them (manual for MVP), then I can reconstruct a trade timeline. [Source: docs/architecture/monitoring-and-observability.md#key-metrics]
...
- [ ] Backend: Add structured logging in canisters for key events (AC: 1) [Source: docs/architecture/monitoring-and-observability.md#key-metrics]
- [ ] Frontend: Append breadcrumbs for trade lifecycle in dev builds (AC: 1) [Source: docs/architecture/monitoring-and-observability.md#monitoring-stack]
- [ ] Documentation: Define event schema and retention policy (AC: 2)

## Dev Notes

- Include fields: `eventType`, `principal`, `entityId`, `timestamp`, `metadata`. [Source: docs/architecture/error-handling-strategy.md#error-response-format]

### Inline Schema Example

Example JSON for a swap refund audit event:

```json
{
  "eventType": "SWAP_REFUND",
  "principal": "aaaaa-aa",
  "entityId": "swap-123",
