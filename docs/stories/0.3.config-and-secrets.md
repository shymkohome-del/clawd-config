...
- Latest QA Review (2025-08-14):
  - Status: Partially met (AC2 met; AC1 not met; AC3 partially met)
  - Evidence:
    - `crypto_market/lib/core/config/app_config.dart`: present; enforces required keys and throws `AppConfigValidationError`
    - `crypto_market/lib/main.dart`: initializes via `AppConfig.load()` and surfaces user-visible error: "Missing required config: <KEY>"
    - Tests: `crypto_market/test/unit/app_config_test.dart` covers missing-key error, valid load, and optional keys nullability
    - `README.md`: has Config & Secrets section with `--dart-define` example and behavior; missing explicit rotation strategy and local/dev/prod conventions
    - `.env.example`: still missing at repo root
  - Gaps vs Acceptance Criteria: AC1 not met; AC3 missing rotation strategy and environment conventions
  - Risk: Developers lack a template for required keys and rotation guidance; setup friction and misconfiguration risk remain
  - Actions to pass:
    - Add `.env.example` with keys listed in AC1 (optional keys commented)
    - Update `README.md` to state rotation strategy and local/dev/prod conventions (where values live, how to rotate, and who owns rotation)
  - Re-test: Confirm `.env.example` exists with all keys; README updated per above
