...
- Evidence:
  - `crypto_market/lib/core/blockchain/icp_service.dart` exists with `ICPService.fromConfig(AppConfig)` factory and TODO for actor init. Methods `register`, `loginWithOAuth`, `getUserProfile` are present and return `Result<..., AuthError>`.
  - `crypto_market/lib/core/blockchain/errors.dart` defines `Result<T,E>` and `AuthError` enum. No explicit mapping helpers implemented yet.
  - `crypto_market/lib/features/auth/providers/auth_service_provider.dart` and `crypto_market/lib/features/market/providers/market_service_provider.dart` exist and accept an `ICPService` instance; app-root DI wiring not shown.
  - Tests for actor init/error mapping not found.
- Acceptance Criteria Review:
  - AC1: Met — service skeleton and init hook point present via factory.
  - AC2: Met — method stubs implemented.
  - AC3: Not Met — domain error mapping utilities/logic missing.
  - AC4: Partially Met — provider wrappers exist, but injection at app start not demonstrated.
- Actions to Pass:
  - Implement error mapping utilities (e.g., `mapAuthExceptionToAuthError`) and apply within `register`, `loginWithOAuth`, and `getUserProfile` with proper try/catch around actor calls.
  - Add app-root DI wiring to construct `ICPService.fromConfig(AppConfig)` and inject into `AuthServiceProvider` and `MarketServiceProvider` (e.g., Bloc `RepositoryProvider` or Riverpod `Provider`).
  - Add unit tests covering actor initialization path and error mapping branches; add a widget test verifying providers obtain the injected service without runtime errors.
  - Ensure conformance with Candid types per `docs/architecture/api-specification.md` references in method stubs.
  - Re-run CI to confirm all tests pass.
