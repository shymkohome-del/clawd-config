...
- [ ] Frontend: Refund UI (AC: 1, 2)
  ...
  - [ ] **Localization: Implement multi-language support for timeout countdown labels, refund button text, error messages for early refund attempts, and refund success confirmations.**
  - [ ] **Logging: Implement debug logging for refund requests, timelock expiry checks, refund eligibility validation, transaction processing, and refund completion events. Ensure logging is disabled in production flavors and only active in development/staging builds.**
