# Story 4.1: Payment method capture & confi- [ ] Frontend: Trade confirmation UI (AC: 2)
  - [ ] Add "Payment received" control on `SwapDetailScreen`. [Source: architecture/frontend-architecture.md#flutter-app-structure]
  - [ ] **Localization: Implement multi-language support for payment confirmation prompts, status updates, and completion notifications.**ation
...
- [ ] Frontend: Payment methods UI and provider (AC: 1)
  ...
  - [ ] **Localization: Implement multi-language support for payment method labels, form fields, validation messages, and method type descriptions.**
  - [ ] **Logging: Implement debug logging for payment method additions, modifications, validation events, confirmation actions, and trade state updates. Ensure logging is disabled in production flavors and only active in development/staging builds.**
