...
- AC1 — Retries/backoff: Implemented. All workflow API calls wrapped with `scripts/retry.sh`, honoring `Retry-After` and 5xx/429/secondary rate limit.
...
- AC3 — Error taxonomy/messages: Implemented. Standardized reasons across steps; surfaced via outputs and summaries (`unsupported-branch`, `status-not-done`, `insufficient-permissions`, `repo-allow-auto-merge-disabled`, `label-gate-missing`, `no-open-pr`, `no-node-id`, `graphql-error`, `enabled`).
- AC4 — Observability/summary: Implemented. Added log groupings, notices/warnings/errors for misconfigurations, and final summaries in both workflows.

Verdict: Ready for Review. Please validate in CI with live GitHub API responses.
