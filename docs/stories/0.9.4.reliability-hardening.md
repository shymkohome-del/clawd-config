...
| 2025-08-20 | 0.6 | Dev: Wired `scripts/retry.sh` into REST/GraphQL call sites in reusable and fallback workflows; added grouped logs and final job summaries; taxonomy unified and surfaced via outputs and summaries. All ACs now satisfied. Status set to Review. | Dev |

## Dev Agent Record

- Current state:
  - Concurrency guard present and verified in workflows
  - `scripts/retry.sh` integrated into REST/GraphQL call sites in `.github/workflows/reusable-auto-pr.yml` and `merge-on-green-fallback.yml`
  - Grouped logs (`::group::`) and final job summaries added
  - Error taxonomy standardized (e.g., `status-not-done`, `unsupported-branch`, `insufficient-permissions`, `automerge.setting_disabled`, `label-gate`, `no-pr`, `no-node-id`)
