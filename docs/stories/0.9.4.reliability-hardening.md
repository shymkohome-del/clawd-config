...
- AC1 — Retries/backoff: Partial. `merge-on-green-fallback.yml` polls checks with simple backoff; not extracted as reusable helper and not applied to all API calls.
- AC2 — Concurrency guard: Pass. Present and consistent across key workflows.
- AC3 — Error taxonomy/messages: Partial. Basic reasons logged; not yet standardized across workflows.
- AC4 — Observability/summary: Partial. Fallback emits a minimal summary; grouping and annotations are limited.

Verdict: Keep `Status: InProgress`. Next actions: add reusable backoff helper, unify error taxonomy, and expand job summaries/annotations.
