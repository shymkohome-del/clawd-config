...
| 2025-08-20 | 0.5 | Dev: Added grouped logs and summary; surfaced outputs `attempted/enabled/reason`; gated auto-merge on `automerge-ok` for story branches; fallback merge-on-green remains source of truth when repo auto-merge setting is disabled. AC1/3/4/5 satisfied; AC2 depends on repo settings outside code. Status set to Review. | Dev |
...
- Blockers:
  - Repo setting “Allow auto-merge” remains disabled; GraphQL enable returns non-fatal error and we fall back to REST-based merge on green
- Next:
  - Keep fallback as source of truth; when repo enables auto-merge, GraphQL path will auto-enable with SQUASH
