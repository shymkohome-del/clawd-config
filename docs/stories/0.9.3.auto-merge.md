...
| 2025-08-19 | 0.4 | Dev: Standardized reason keys and expanded job summaries; added outputs for auto-merge enablement (attempted/enabled/reason); fixed YAML lint; validated with `scripts/dev-validate.sh`; committed on `story/0.9.3-auto-merge`. | Dev |
...
- Implemented:
  ...
  - Standardized reason keys across workflows; exposed outputs attempted/enabled/reason; YAML lint fixed and validated locally
- Blockers:
