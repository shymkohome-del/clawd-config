...
| 2025-08-20 | 0.5 | Dev: Fixed workflow lint (yamllint empty-lines), aligned fallback check-run name to wrapper job, confirmed Flutter gates green locally. GraphQL enable remains guarded and exits gracefully when repo auto-merge is disabled. Status stays InProgress pending repo setting. | Dev |
| 2025-08-20 | 0.6 | Dev: Confirmed repo setting “Allow auto-merge” is ENABLED. No further prompts needed. Proceed with GraphQL auto-merge enablement path where eligible; fallback remains as safety net. | Dev |
...
- Blockers:
  - None. Repo setting “Allow auto-merge” is enabled.
- Next:
  - Prefer GraphQL enable path; monitor enablement outcomes on eligible PRs (label `automerge-ok` for `story/*`). Fallback merge-on-green remains as safety net.

## QA Results

- AC1 — GraphQL enable auto-merge: Partial. Step present and guarded; outputs expose attempted/enabled/reason; blocked by repo auto-merge setting.
