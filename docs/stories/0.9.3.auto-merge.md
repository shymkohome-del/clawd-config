...
| 2025-08-19 | 0.4 | QA: Updated — Auto-merge step now exposes `attempted/enabled/reason` outputs; repo auto-merge disabled blocks enablement. AC1 Partial, AC2 Partial, AC3 Pass, AC4 Pass, AC5 Partial. Status InProgress. | QA |
| 2025-08-20 | 0.5 | QA: Repo “Allow auto‑merge” confirmed enabled. AC1 Pass, AC2 Pass, AC3 Pass, AC4 Pass, AC5 Pass. Status set to Done. | QA |
| 2025-08-20 | 0.6 | Dev verification: Ran `flutter analyze --fatal-infos --fatal-warnings` and `flutter test --no-pub`; all passed. Status 'Done' validated. No code changes made. | Dev |
...
- AC1 — GraphQL enable auto-merge: Pass. Step present and guarded; GraphQL `enablePullRequestAutoMerge` with `SQUASH` succeeds when label gate is satisfied.
- AC2 — Repo settings: Pass. “Allow auto‑merge” is enabled and squash allowed on `develop`.
...
- AC5 — Auto-merge outcome: Pass. PRs merge automatically when checks pass; source branch deletion attempted per rules.

Verdict: `Status: Done`. All ACs satisfied.
...
- Repo settings validation:
  - Verify repo-level `allow_auto_merge` is true via REST; document how to remediate when false.
- Label gate behavior:
  - On `story/*` without `automerge-ok`, ensure `attempted=false` and `reason=label-gate-missing`; with label present, `enabled=true` or actionable reason when not.
- GraphQL/REST parity:
  - If GraphQL enable fails, fallback `merge-on-green-fallback.yml` merges when checks are green; verify both code paths across two PRs.
- Source branch cleanup:
  - After merge, confirm source branch is deleted when permitted; protected branches are skipped safely.
- Observability:
  - Ensure summary includes `attempted`, `enabled`, and `reason` values; reruns should produce consistent summaries.
