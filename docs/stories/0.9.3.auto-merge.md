...
- [ ] Add/ensure GraphQL step calls `enablePullRequestAutoMerge` on the PR node id with `mergeMethod: SQUASH`.
- [ ] Add logic to gate auto‑merge enablement on presence of `automerge-ok` label, except for non‑story branches which are always allowed.
- [ ] Verify repo settings: allow auto‑merge globally, allow `squash` on `develop`, and define required checks to avoid accidental merges.
- [ ] On settings/permission failure, log a clear note and continue without failing the job.
...
- AC1 — GraphQL enable auto-merge: Partial. Step present and guarded; returns non-fatal error when repo setting disallows auto-merge.
- AC2 — Repo settings: Partial. Required checks configured; need confirmation that “Allow auto-merge” is enabled and squash merges allowed on `develop`.
- AC3 — Label gate: Pass. `automerge-ok` required for `story/*`; non-story branches allowed.
- AC4 — Clear logs on failure: Pass. On failure to enable auto-merge, logs a non-failing note and continues.
- AC5 — Auto-merge outcome: Partial. Fallback merge-on-green merges when checks pass; GraphQL path pending repo setting.

Verdict: Keep `Status: InProgress`. Next actions: Enable repo “Allow auto-merge” or accept fallback as primary; verify squash allowed on `develop`.
