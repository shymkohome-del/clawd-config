...
| 2025-08-20 | 0.5 | Dev: Reusable workflow now exposes repo settings via GraphQL (autoMergeAllowed, squashMergeAllowed, develop required checks) and emits an auto-merge summary. Enable step outputs `attempted/enabled/reason`. Local format/analyze/tests passed. | Dev |
...
- Implemented:
  ...
  - Repo settings probe (non-blocking): records auto-merge/squash flags and `develop` required checks; summary emitted
- Blockers:
