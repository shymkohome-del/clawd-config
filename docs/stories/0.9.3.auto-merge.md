...
| 2025-08-20 | 0.5 | Dev: Exposed reusable outputs (`auto_merge_attempted/enabled/reason`), aligned fallback required check name, hardened local validator yamllint fallback. Format/analyze/tests passed locally. Set Status: Review. | Dev |
...
- Implemented:
  ...
  - Reusable workflow now exposes `auto_merge_attempted`, `auto_merge_enabled`, and `auto_merge_reason` outputs for observability
- Blockers:
...
- Next:
  - Keep fallback as source of truth; when repo enables auto-merge, prefer GraphQL enable path. Coordinate with repo admins to enable “Allow auto‑merge” and permit squash on `develop`.
