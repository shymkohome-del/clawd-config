...
<<<<<<< HEAD
- Evidence:
  - UI and state flow implemented (`ProfileScreen`, `ProfileCubit`, `UserProfile`)
  - UserServiceProvider uses placeholders/TODOs for canister calls and IPFS; persistence simulated
  - Reputation display implemented; updates not persisted to backend

- Acceptance Criteria:
  - [ ] AC1: Persist profile updates → NOT met (no real canister persistence)
  - [x] AC2: Display reputation score → Met (display-only)
  - [ ] AC3: IPFS-backed image storage → NOT met (simulated only)

- Actions to pass next iteration:
  - Implement real canister calls for `getUserProfile`/`updateUserProfile` and wire into `UserServiceProvider`
  - Integrate an IPFS client; store returned hash and fetch via gateway
  - Add/update tests to cover real persistence paths (mock canister client or local replica)
=======
_Pending QA_
>>>>>>> origin/codex/develop-profile-and-reputation-feature

- Status Decision: InProgress until persistence and IPFS integration are delivered
